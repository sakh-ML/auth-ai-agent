...
def handle_login_system_prompt() -> str:
    ...
    You will be provided with the target website's HTML DOM and the user's credentials.
    ...
    2. Use the `write_in_field` tool to enter the provided username.
    3. Use the `write_in_field` tool to enter the provided password.
...
def handle_2fa_system_prompt() -> str:
    ...
    You will be provided with the target website's HTML DOM and the current one-time verification code.
    ...
    2. Use the `write_in_field` tool to enter the provided verification code.
    ...
    - Do NOT invent or hallucinate selectors.
    - Use the verification code exactly as provided in the prompt.
...
def handle_login_prompt(dom: str) -> str:
    ...
    Use your tools to:
    1. Write the placeholder `USERNAME` into the username/email field.
    2. Write the placeholder `PASSWORD` into the password field.
    3. Click the login button.

    IMPORTANT:
    - ALWAYS use the placeholder `USERNAME` for the username/email field.
    - ALWAYS use the placeholder `PASSWORD` for the password field.
...
def handle_2fa_prompt(dom: str) -> str:
    ...
    if not totp_secret:
        raise RuntimeError("Unexcpcted error TOTP_SECRET is not set")

    try:
        totp = pyotp.TOTP(totp_secret)
        logger.info(f"Current OTP: {totp.now()}")
    except Exception as e:
        logger.error(f"Failed to get current OTP: {e}")
        raise

    return f"""
    Here are the credentials you need to use:
    This is the current OTP
    {totp.now()}
    ...
    Use your tools to write the TOTP into the correct fields and click the submit button.
    """.strip()
...
def get_real_value(page_url: str, place_holder: str) -> str | None:
    hostname = urlparse(url=page_url).hostname
    if hostname is None:
        logger.warning(f"Hostname is None for {page_url}")
        return None
    ...
    creds = credentials.get(hostname)
    if creds is None:
        logger.warning(f"No Credintals found for: {page_url}")
        return None
    ...
    match place_holder:
        case "USERNAME":
            ...
            if username is None:
                logger.warning(f"Username is None for {page_url}")
                return None
            return username
        case "PASSWORD":
            password = creds.get("password")
            if password is None:
                logger.warning(f"Password is None for {page_url}")
                return None
            return password
        case _:
            return None
...
async def execute_function_call(page, function: FunctionLLMTool, arguments):
    logger.info(f"Executing: {function.value}, with arguemnts: {arguments}")

    match function:
        case FunctionLLMTool.CLICK_ELEMENT:
            await click_element(page, arguments["selector"])
        case FunctionLLMTool.WRITE_IN_FIELD:
            real_value = get_real_value(page.url, arguments["value"])
            if real_value is None:
                placeholder = arguments["value"]
                logger.error("Calling write in field failed")
                logger.error(
                    f"Error getting the real value from a placeholder: {placeholder}"
                )
                return
            logger.info(f"Real value: {real_value}")
...
async def handle_2fa(page):
    ...
    avaiable_functions = [f.value for f in FunctionLLMTool]
    logger.info(f"Avalaible fucntions: {avaiable_functions}")

    logger.info("Check which tools the agent need, and call them")
    for item in response.output:
        if item.type == "function_call":
            if item.name in avaiable_functions:
...
async def on_page_load_openai(page):
    ...
    logger.info(f"New url loaded: {current_url}")

    if not is_valid_url(current_url):
        logger.warning(
            f"The new url loadded is invalid: {current_url}, skip authentication ..."
        )
        return

    logger.info("-" * 50 + "\n")

    logger.info("Asking Client if this is an login page")

    if not await is_login_page(page):
        logger.info(f"Not a login page, skip authentication ...")
        return

    logger.info("Handle the loging into page")

    await handle_login(page)

    logger.info("Handling login in the page completed")

    totp_secret = os.getenv("TOTP_SECRET")
    if not totp_secret:
        logger.warning("TOTP_SECRECT is not set skip check TOTP authentication")
        return

    logger.info("TOTP_SECRECT is set, ask client if the page need TOTP handling ...")

    logger.info("Wait tell networkidle")
    await page.wait_for_load_state("networkidle")

    if not await is_2fa_page(page):
        logger.info("Page dosent require 2fa authentication, skip ...")
        return
    ...
    logger.info("Handling 2fa completed")
